...
class InterfaceAllowedIpsValidator:
    def __call__(self, form, field):
        if not field.data:
            return
        try:
            ips = json.loads(field.data)
        except json.JSONDecodeError:
            return stop_validation(field, "must be a valid JSON array.")
        ...
        for ip in ips:
            try:
                ...
                return stop_validation(field, f"'{ip}' is not a valid IPv4 address. Follow the format 'X.X.X.X/Y'.")
        if Interface.is_allowed_ips_in_use(ips, form.iface):
            return stop_validation(field, "address already in use!")


class InterfaceAllowedIpsMultipleValidator:
    def __call__(self, form, field):
        if not field.data:
            return
        try:
