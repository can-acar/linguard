...
class AddInterfaceForm(FlaskForm):
    ...
    # return list(set(get_system_interfaces().keys()) - set(["lo"]))
    @classmethod
    def allowed_gateways(cls) -> List[str]:
        # get routing table ip addresses and remove loopback
        table = json.loads(Command("ip -json route").run().output)
        gateways = []
        for route in table:
            if route["dst"] == "default":
                continue
            if route["dst"] not in gateways:
                gateways.append(route["dst"])
        return gateways
